...
import { InverterModel, InverterBrand, getInverterModelsForBrand, findInverterModel, findInverterBrand, INVERTER_BRANDS } from '@/data/compare-inverters'

interface InverterCompareSelectorProps {
  onCompare: (inverter1: InverterModel, brand1: InverterBrand, inverter2: InverterModel, brand2: InverterBrand) => void
...
export default function InverterCompareSelector({
  onCompare,
  initialBrand1 = '',
  initialModel1 = '',
  initialBrand2 = '',
  initialModel2 = '',
}: InverterCompareSelectorProps) {
  ...
  const runComparison = (b1: string, m1: string, b2: string, m2: string) => {
    ...
    const p1 = findInverterModel(b1, m1)
    const brandObj1 = findInverterBrand(b1)
    const p2 = findInverterModel(b2, m2)
    const brandObj2 = findInverterBrand(b2)
  ...
  const renderBrandOptions = () => (
    <>
      <optgroup label="👑 Premium">
        {INVERTER_BRANDS.filter(b => b.tier === 'premium').map(b => (
          <option key={b.brandSlug} value={b.brandSlug}>{b.brandName}</option>
        ))}
      </optgroup>
      <optgroup label="🥇 Tier 1 Hybrid">
        {INVERTER_BRANDS.filter(b => b.tier === 'tier1').map(b => (
          <option key={b.brandSlug} value={b.brandSlug}>{b.brandName}</option>
        ))}
      </optgroup>
      <optgroup label="🥈 Tier 2">
        {INVERTER_BRANDS.filter(b => b.tier === 'tier2').map(b => (
          <option key={b.brandSlug} value={b.brandSlug}>{b.brandName}</option>
        ))}
      </optgroup>
      <optgroup label="💰 Budget">
        {INVERTER_BRANDS.filter(b => b.tier === 'budget').map(b => (
          <option key={b.brandSlug} value={b.brandSlug}>{b.brandName}</option>
        ))}
      </optgroup>
    </>
  ...
  const b1Model = findInverterModel(brand1, model1)
  const b2Model = findInverterModel(brand2, model2)
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-end">
        ...
        <div className="space-y-4">
          ...
          >
            ...
            {getInverterModelsForBrand(brand1).map(m => (
        ...
        <div className="space-y-4">
          ...
          >
            <option value="">Select model</option>
            {getInverterModelsForBrand(brand2).map(m => (
              <option key={m.value} value={m.value}>
                {m.label}
              </option>
            ))}
      ...
      {b1Model && b2Model && (
        <div className="mt-8 border-t border-border pt-6 flex flex-col md:flex-row justify-between items-center text-sm gap-4 bg-gray-50 rounded-xl p-4">
          <div className="text-center w-full">
            <div className="font-semibold text-text-primary">{b1Model.efficiency}% efficiency</div>
            <div className="text-text-muted">{b1Model.switchoverTimeMs}ms switchover</div>
          ...
          <div className="text-center w-full">
            <div className="font-semibold text-text-primary">{b2Model.efficiency}% efficiency</div>
            <div className="text-text-muted">{b2Model.switchoverTimeMs}ms switchover</div>
            <div className="text-primary font-medium">
