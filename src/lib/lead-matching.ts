...
export async function rematchUnmatchedLeads(
  ...
  let query = supabase
    .from("leads")
    .select("id, full_name, phone, whatsapp, email, state, city, monthly_bill_range, timeline, lead_type")
  ...
  for (const lead of unmatchedLeads) {
    ...
    const installers = await findInstallersForState(lead.state, limit);
    ...
    const { error: updateError } = await supabase
      .from("leads")
      .update({ installer_id: bestInstaller.id, status: "new" })
      .eq("id", lead.id);
...
async function sendNotificationsForRematch(
  ...
  if (installer.whatsapp || installer.phone) {
    promises.push(
      sendInstallerLeadNotification(installer.whatsapp || installer.phone!, {
  ...
  if (installer.email) {
    promises.push(
      sendInstallerLeadEmail(installer.email, installer.company_name, {
        full_name: lead.full_name,
        city: lead.city || lead.state,
        state: lead.state,
        monthly_bill_range: lead.monthly_bill_range || "Not specified",
