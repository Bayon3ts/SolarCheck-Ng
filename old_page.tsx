...
export default function LoginPage() {
  ...
  const handleForgotPassword = async () => {
    ...
    setEmailError('Γ£ô Password reset link sent to your email');
  };
  ...
  return (
    <>
      <style>{`
        ...
        /* ΓöÇΓöÇ Full-bleed background ΓöÇΓöÇ */
        .login-bg {
          position: fixed;
          inset: 0;
        ...
        /* ΓöÇΓöÇ Animated solar orb (top-left) ΓöÇΓöÇ */
        .orb-wrap {
          position: fixed;
          top: 48px;
          left: 48px;
        ...
        /* ΓöÇΓöÇ Logo watermark (top-center always) ΓöÇΓöÇ */
        .login-logo {
          position: fixed;
          top: 32px;
          left: 50%;
      ...
      {/* ΓöÇΓöÇ Animated solar orb (desktop only, top-left) ΓöÇΓöÇ */}
      ...
      <div style={{
        ...
        <div className="login-card anim-up">
          ...
          <div className="anim-up delay-200" style={{
            ...
            {([
              { id: 'email', label: 'Γ£ë∩╕Å  Email & Password' },
              { id: 'whatsapp', label: '≡ƒÆ¼  WhatsApp OTP' },
            ] as const).map((tab) => (
              <button
                key={tab.id}
            ...
            <div className="anim-fade" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {emailError && (
                <div className="anim-up" style={{
                  ...
                  background: emailError.includes('Γ£ô') ? '#F0FDF4' : '#FEF2F2',
                  border: `1px solid ${emailError.includes('Γ£ô') ? '#BBF7D0' : '#FCA5A5'}`,
                  color: emailError.includes('Γ£ô') ? '#166534' : '#B91C1C',
                }}>
                  {!emailError.includes('Γ£ô') && <AlertTriangle style={{ width: '15px', height: '15px', flexShrink: 0, marginTop: '1px' }} />}
              ...
              {!otpSent ? (
                <>
                  ...
                  <button onClick={handleSendOtp} disabled={loading || whatsappNumber.length < 10} className="btn-p">
                    {loading && <Loader2 style={{ width: '16px', height: '16px', animation: 'spin 1s linear infinite' }} />}
                    {loading ? 'SendingΓÇª' : 'Send WhatsApp Code'}
                    {!loading && <ArrowRight style={{ width: '16px', height: '16px' }} />}
                  </button>
                </>
              ) : (
                <>
                  <div style={{ textAlign: 'center', padding: '8px 0' }}>
                    <div style={{ fontSize: '38px', marginBottom: '10px' }}>≡ƒô▒</div>
                  ...
                  <button onClick={handleVerifyOtp} disabled={loading || otpCode.length < 6} className="btn-p">
                    {loading && <Loader2 style={{ width: '16px', height: '16px', animation: 'spin 1s linear infinite' }} />}
                    {loading ? 'VerifyingΓÇª' : 'Verify & Sign In'}
                    {!loading && <ArrowRight style={{ width: '16px', height: '16px' }} />}
                  </button>
          ...
          <p style={{ textAlign: 'center', fontSize: '12px', color: '#9CAB9E', marginTop: '18px' }}>
            ≡ƒöÆ Your data is encrypted and secure
          </p>
        </div>
      </div>
    </>
